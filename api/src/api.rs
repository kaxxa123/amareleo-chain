...
impl AmareleoApi {
    ...
    pub async fn start<N: Network, C: ConsensusStorage<N>>(
        &self,
        shutdown: Arc<AtomicBool>,
    ) -> Result<(Validator<N, C>, Option<String>)> {
        ...
        let log_file: Option<String> = if !self.log_mode.is_none() {
            let log = start_logger::<N>(
                self.log_mode.get_path().clone(),
                &tag,
                self.log_mode.is_stdout(),
                self.keep_state,
                self.verbosity,
                shutdown.clone(),
            )?;
...
fn start_logger<N: Network>(
    ...
    let log_path = match logfile.clone() {
        Some(path) => path,
        None => amareleo_log_file(N::ID, keep_state, end_point_tag),
    };

    // Initialize the logger.
    crate::helpers::initialize_logger(verbosity, log_path.clone(), stdout_log, shutdown);
