...
pub use snarkos_lite_node_router_messages as messages;
...
use snarkos_lite_node_tcp::{is_bogon_ip, is_unspecified_or_broadcast_ip, Config, Tcp, P2P};
...
impl<N: Network> Router<N> {
    ...
    pub fn local_ip(&self) -> SocketAddr {
        self.tcp
            .listening_addr()
            .expect("The TCP listener is not enabled")
    }
    ...
    pub fn is_connected_validator(&self, peer_ip: &SocketAddr) -> bool {
        self.connected_peers
            .read()
            .get(peer_ip)
            .map_or(false, |peer| peer.is_validator())
    }
    ...
    pub fn is_connected_prover(&self, peer_ip: &SocketAddr) -> bool {
        self.connected_peers
            .read()
            .get(peer_ip)
            .map_or(false, |peer| peer.is_prover())
    }
    ...
    pub fn is_connected_client(&self, peer_ip: &SocketAddr) -> bool {
        self.connected_peers
            .read()
            .get(peer_ip)
            .map_or(false, |peer| peer.is_client())
    }
    ...
    pub fn connected_validators(&self) -> Vec<SocketAddr> {
        self.connected_peers
            .read()
            .iter()
            .filter(|(_, peer)| peer.is_validator())
            .map(|(ip, _)| *ip)
            .collect()
    }
    ...
    fn update_metrics(&self) {
        metrics::gauge(
            metrics::router::CONNECTED,
            self.connected_peers.read().len() as f64,
        );
        metrics::gauge(
            metrics::router::CANDIDATE,
            self.candidate_peers.read().len() as f64,
        );
        metrics::gauge(
            metrics::router::RESTRICTED,
            self.restricted_peers.read().len() as f64,
        );
    }
    ...
    pub fn update_connected_peer<Fn: FnMut(&mut Peer<N>)>(
        ...
        if let Some(peer) = self.connected_peers.write().get_mut(&peer_ip) {
            ...
            if peer.node_type() != node_type {
                bail!(
                    "Peer '{peer_ip}' has changed node types from {} to {node_type}",
                    peer.node_type()
                )
            }
