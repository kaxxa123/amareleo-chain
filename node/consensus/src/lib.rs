...
use snarkos_lite_node_bft::{
    helpers::{
        fmt_id, init_consensus_channels, ConsensusReceiver, PrimaryReceiver, PrimarySender,
        Storage as NarwhalStorage,
    },
    spawn_blocking, Primary, BFT, MAX_BATCH_DELAY_IN_MS,
};
...
impl<N: Network> Consensus<N> {
    ...
    pub async fn run(
        ...
        self.primary_sender
            .set(primary_sender.clone())
            .expect("Primary sender already set");
        ...
        self.bft
            .run(Some(consensus_sender), primary_sender, primary_receiver)
            .await?;
        Ok(())
    }
...
impl<N: Network> Consensus<N> {
    ...
    pub fn unconfirmed_transmission_ids(&self) -> impl '_ + Iterator<Item = TransmissionID<N>> {
        self.worker_transmission_ids()
            .chain(self.inbound_transmission_ids())
    }

    /// Returns the unconfirmed transmissions.
    pub fn unconfirmed_transmissions(
        &self,
    ) -> impl '_ + Iterator<Item = (TransmissionID<N>, Transmission<N>)> {
        self.worker_transmissions()
            .chain(self.inbound_transmissions())
    }

    /// Returns the unconfirmed solutions.
    pub fn unconfirmed_solutions(
        &self,
    ...
    pub fn unconfirmed_transactions(
        &self,
    ) -> impl '_ + Iterator<Item = (N::TransactionID, Data<Transaction<N>>)> {
        self.worker_transactions()
            .chain(self.inbound_transactions())
    }
}
...
impl<N: Network> Consensus<N> {
    ...
    pub fn inbound_solutions(
        &self,
    ) -> impl '_ + Iterator<Item = (SolutionID<N>, Data<Solution<N>>)> {
        // Return an iterator over the solutions in the inbound queue.
        self.solutions_queue
            .lock()
            .clone()
            .into_iter()
            .map(|(id, solution)| (id, Data::Object(solution)))
    }

    /// Returns the transactions in the inbound queue.
    pub fn inbound_transactions(
        &self,
    ) -> impl '_ + Iterator<Item = (N::TransactionID, Data<Transaction<N>>)> {
...
impl<N: Network> Consensus<N> {
    ...
    pub async fn add_unconfirmed_solution(&self, solution: Solution<N>) -> Result<()> {
        ...
        let checksum =
            Data::<Solution<N>>::Buffer(solution.to_bytes_le()?.into()).to_checksum::<N>()?;
        #[cfg(feature = "metrics")]
        {
        ...
        // Queue the unconfirmed solution.
        {
            ...
            if self
                .ledger
                .contains_transmission(&TransmissionID::Solution(solution_id, checksum))?
            {
                bail!(
                    "Solution '{}' exists in the ledger {}",
                    fmt_id(solution_id),
                    "(skipping)".dimmed()
                );
            }
            ...
            trace!(
                "Received unconfirmed solution '{}' in the queue",
                fmt_id(solution_id)
            );
            if self
                .solutions_queue
                .lock()
                .put(solution_id, solution)
                .is_some()
            {
                bail!(
                    "Solution '{}' exists in the memory pool",
                    fmt_id(solution_id)
                );
            }
        }
    ...
    pub async fn process_unconfirmed_solutions(&self) -> Result<()> {
        ...
        let solutions = {
            ...
            (0..num_solutions)
                .filter_map(|_| queue.pop_lru().map(|(_, solution)| solution))
                .collect::<Vec<_>>()
        };
        ...
        for solution in solutions.into_iter() {
            let solution_id = solution.id();
            trace!(
                "Adding unconfirmed solution '{}' to the memory pool...",
                fmt_id(solution_id)
            );
            // Send the unconfirmed solution to the primary.
            if let Err(e) = self
                .primary_sender()
                .send_unconfirmed_solution(solution_id, Data::Object(solution))
                .await
            {
    ...
    pub async fn add_unconfirmed_transaction(&self, transaction: Transaction<N>) -> Result<()> {
        // Calculate the transmission checksum.
        let checksum =
            Data::<Transaction<N>>::Buffer(transaction.to_bytes_le()?.into()).to_checksum::<N>()?;
        #[cfg(feature = "metrics")]
        {
            metrics::increment_gauge(metrics::consensus::UNCONFIRMED_TRANSACTIONS, 1f64);
            let timestamp = snarkos_lite_node_bft::helpers::now();
            self.transmissions_queue_timestamps.lock().insert(
                TransmissionID::Transaction(transaction.id(), checksum),
                timestamp,
            );
        }
        // Queue the unconfirmed transaction.
        {
            ...
            if transaction.is_fee() {
                bail!(
                    "Transaction '{}' is a fee transaction {}",
            ...
            if self
                .ledger
                .contains_transmission(&TransmissionID::Transaction(transaction_id, checksum))?
            {
                bail!(
            ...
            trace!(
                "Received unconfirmed transaction '{}' in the queue",
                fmt_id(transaction_id)
            );
    ...
    pub async fn process_unconfirmed_transactions(&self) -> Result<()> {
        ...
        let transactions = {
            ...
            let selector_iter = (0..num_deployments)
                .map(|_| true)
                .interleave((0..num_executions).map(|_| false));
        ...
        for transaction in transactions.into_iter() {
            let transaction_id = transaction.id();
            trace!(
                "Adding unconfirmed transaction '{}' to the memory pool...",
                fmt_id(transaction_id)
            );
            // Send the unconfirmed transaction to the primary.
            if let Err(e) = self
                .primary_sender()
                .send_unconfirmed_transaction(transaction_id, Data::Object(transaction))
                .await
            {
                // If the BFT is synced, then log the warning.
                if self.bft.is_synced() {
...
impl<N: Network> Consensus<N> {
    ...
    fn start_handlers(&self, consensus_receiver: ConsensusReceiver<N>) {
        let ConsensusReceiver {
            mut rx_consensus_subdag,
        } = consensus_receiver;

        // Process the committed subdag and transmissions from the BFT.
        let self_ = self.clone();
        self.spawn(async move {
            while let Some((committed_subdag, transmissions, callback)) =
                rx_consensus_subdag.recv().await
            {
                self_
                    .process_bft_subdag(committed_subdag, transmissions, callback)
                    .await;
            }
        });
    ...
    fn try_advance_to_next_block(
        ...
        #[cfg(feature = "metrics")]
        {
            let elapsed = std::time::Duration::from_secs(
                (snarkos_lite_node_bft::helpers::now() - start) as u64,
            );
            ...
            metrics::add_transmission_latency_metric(
                &self.transmissions_queue_timestamps,
                &next_block,
            );

            metrics::gauge(
                metrics::consensus::COMMITTED_CERTIFICATES,
                num_committed_certificates as f64,
            );
            metrics::histogram(
                metrics::consensus::CERTIFICATE_COMMIT_LATENCY,
                elapsed.as_secs_f64(),
            );
            ...
            metrics::gauge(
                metrics::blocks::CUMULATIVE_PROOF_TARGET,
                cumulative_proof_target as f64,
            );
        }
        Ok(())
    }
    ...
    async fn reinsert_transmissions(
        &self,
        transmissions: IndexMap<TransmissionID<N>, Transmission<N>>,
    ) {
        ...
        for (transmission_id, transmission) in transmissions.into_iter() {
            ...
            if let Err(e) = self
                .reinsert_transmission(transmission_id, transmission)
                .await
            {
                warn!(
        ...
        match (transmission_id, transmission) {
            ...
            (TransmissionID::Solution(solution_id, _), Transmission::Solution(solution)) => {
                ...
                self.primary_sender()
                    ...
                    .send((solution_id, solution, callback))
                    .await?;
            }
            (
                ...
                self.primary_sender()
                    ...
                    .send((transaction_id, transaction, callback))
                    .await?;
            }
